...
class Gen_SSimu(Ui_SSimu):
    def setupUi(self, SSimu):
        ...
        # Setup of in_T_mag
        txt = self.tr(u"""Permanent magnet temperature to adapt magnet remanent flux density""")
        ...
        # Setup of is_per_a
        txt = self.tr(u"""True to compute only on one angle periodicity (use periodicities defined in axes_dict[angle])""")
        ...
        # Setup of is_per_t
        txt = self.tr(u"""True to compute only on one time periodicity (use periodicities defined in axes_dict[time])""")
        self.is_per_t.setWhatsThis(txt)
        self.is_per_t.setToolTip(txt)

        # Setup of in_Kmesh
        txt = self.tr(u"""global coefficient to adjust mesh fineness in FEMM (1 : default , > 1 : finner , < 1 : less fine)""")
        ...
        txt = self.tr(u"""To run FEMM in parallel (the parallelization is on the time loop)""")
